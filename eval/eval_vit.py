...
os.environ["CUDA_VISIBLE_DEVICES"] = str(args.device % 4)
...
def test(test_loader,device,eps,source_model,target_model,attack_name,iter_num,beta,decay_factor,Tpt,N,S,n,lr,N_vt,beta_vt):
    ...
    for data,name,target,TargetClass in test_loader:
        ...
        if perturbed_label.item() == target.item() - 1:
            correct+=1
            # acc_list.append(0)
        else:
            success+=1
    ...
    # acc_frame.to_excel("../results/atk={},{}->{},accuracy={:.4f}-output.xlsx".format(attack_name,args.source_model,args.target_model,accuracy))

    dic={'resnet_v2_152':1, 'resnet_v2_101':2, 'inception_v3':3,  "resnet_v2_50":4,'vit_large_patch16_224':5 ,"deit_base_patch16_224":6, "deit3_base_patch16_224":7, "resnetv2_101x3_bitm":8}
    excel_file = '/data/huangtao/projects/subsapce-attack/results/excel/' + attack_name + 'result.txt'
    if os.path.exists(excel_file):
        df = np.loadtxt(excel_file,delimiter=",")
        if args.source_model not in dic:
            dic[args.source_model] = len(dic)+1
